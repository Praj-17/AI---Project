...
def  wishMe():
    # subprocess.call(["afplay", "E:\\CODING PLAYGROUND\\CODE\\Projects\\AI Preject\\Alexa_Assistant_Original\\Gallery\\sound.mp3"])
...
def voicename():
    if vno==0:
        ...
        speak("I am your personal assistant, how may i help you today?")
...
def sendEmail(to,content):
    ...
    #Modern email servers use port 587 for the secure submission of email for delivery.
     ...
     server.login(config.email_id,config.var_pass) #Log in on an SMTP server that requires authentication.
     server.sendmail(config.email_id,to,content) #from,to,msg
     server.close()
...
class CallPy(object):
    def __init__(self, path= config.sound_py_file):
        self.path = path
...
def TaskExecution():
    ...
    while True:
        ...
        if "hello" in said:
            ...
            person = said.replace('search', "") or said.replace('wikipedia', "")
        ...
        elif 'programming joke' or 'joke' in said:
            ...
            try:
                ...
                # subprocess.call([opener, excel_address])
                os.startfile(config.excel_address)
            except Exception as  e:
                print(e)
                speak("Can not open Microsoft Excel")
            ...
            opener = "open" if sys.platform == "darwin" else "xdg-open"
            subprocess.call([opener, config.recorderpath])
